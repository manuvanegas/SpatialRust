export Coffee, init_light_spatialrust, createweather, create_farm_map, step_n!
...
function init_light_spatialrust(;
    steps::Int, row_d::Int, plant_d::Int, shade_d::Int, barriers::NTuple{2, Int}, 
    ...
    w = createweather(rain_prob, wind_prob, mean_temp, steps, rng)

    farm_map = create_farm_map(map_side, row_d, plant_d, shade_d, :regular, 1, barriers)

    smap = create_shade_map(farm_map, shade_r, map_side, :none)
    ...
    b = Books(
        0, 0, ind_shade_i(shade_g_rate, max_shade, 0, post_prune, prune_sch),
