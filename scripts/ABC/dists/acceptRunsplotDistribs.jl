...
sortedrows = best_100(dists, true,
    [:P12loss, :LP, :incid, :rusts, :meanlats, :exh, :depsdiff],
    Symbol[]
)
...
pdiss = mkpath("../../Dissertation/Chapters/Diss/Document/Figs/ABC")
# still have to figure out this
save("plots/ABC/sideq16_1200.png", fig12s)
save(joinpath(pdiss,"sideq16_1200.png"), fig12s)
...
toprows = top_n_rows(parameterpriors, sortedrows.p_row, 5)
dfabc = reduce(vcat, map(sim_abc, Tables.namedtupleiterator(toprows)))
testruns = test_params(toprows, 1)
